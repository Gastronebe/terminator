...
import { ArrowLeft, Clock, Scale, Info, Snowflake } from 'lucide-react';
    ...
    return (
        <main className={`container ${styles.container}`}>
            <div className={styles.header}>
                <Link href="/norms/cold" className={styles.backButton}>
                    <ArrowLeft size={24} />
                </Link>
                <div className={styles.titleWrapper}>
                    <h1 className={styles.title}>{category?.name || 'Načítám...'}</h1>
                    <p className={styles.subtitle}>Norma č. {categoryId} (Studená kuchyně)</p>
                </div>
            </div>
