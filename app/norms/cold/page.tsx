...
import { ArrowLeft, Search, Snowflake } from 'lucide-react';

import styles from './page.module.css';

export default function ColdKitchenPage() {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                // 1. Fetch Categories
                const q = query(collection(db, 'normCategories'), where('source', '==', 'cold'), orderBy('order'));
                const querySnapshot = await getDocs(q);
                let cats = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as NormCategory));

                // 2. Fetch All Recipes (for accurate counts)
                const qRecipes = query(collection(db, 'normRecipes'), where('source', '==', 'cold'));
        ...
        <main className={`container ${styles.container}`}>
            <div className={styles.header}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <Snowflake size={32} className="text-cyan-500" />
                    <h1 className={styles.title}>Studená kuchyně</h1>
                </div>
            </div>
                        ...
                        <div key={group}>
                            ...
                            <div className={styles.grid}>
                                {filteredGroups[group].map(cat => (
                                    <Link key={cat.id} href={`/norms/cold/${cat.id}`} className={styles.card}>
